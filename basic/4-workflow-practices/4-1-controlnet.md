...
| Lineart  | control\_v11p\_sd15\_lineart   | 라인 아트 추출    | 애니메이션 스타일      |
| Scribble | control\_v11p\_sd15\_scribble  | 낙서/스케치 제어   | 대략적 윤곽선        |
| Softedge | control\_v11p\_sd15\_softedge  | 부드러운 엣지     | 자연스러운 경계       |
| Seg      | control\_v11p\_sd15\_seg       | 의미론적 세그먼테이션 | 영역별 제어         |
| Normal   | control\_v11p\_sd15\_normalbae | 표면 법선 맵     | 조명/질감 제어       |
...
* **control\_v11p\_sd15\_openpose.pth**: https://huggingface.co/lllyasviel/ControlNet-v1-1/resolve/main/control\_v11p\_sd15\_openpose.pth
...
2. **Load ControlNet Model의 CONTROL\_NET 출력** → **Apply ControlNet의 control\_net 입력**
...
> 처음에는 0.7로 시작하는 것이 안정적입니다. 참조 이미지를 더 정확히 따라야 한다면 1.0으로 올리고, 너무 강하게 적용된다면 0.5로 낮춰보세요.
...
아래는 ComfyUI 공식 예제 사이트의 ControlNet 워크플로우입니다. 이 이미지들을 ComfyUI에 드래그하면 워크플로우를 바로 불러올 수 있습니다.
...
![Scribble 입력 이미지 예시](../../.gitbook/assets/input_scribble_example.png)
...
![Depth Map 입력 이미지 예시](../../.gitbook/assets/shark_depthmap.png)
...
**A**: 참조 이미지에서 인물의 포즈가 명확하게 보이는지 확인하고, 전처리기의 detect\_body/hand/face 설정을 조정해보세요.

### Q: 생성된 이미지가 참조 이미지와 너무 달라요

**A**: strength 값을 높이거나 (0.8\~1.0), cfg\_scale을 조정해보세요.
